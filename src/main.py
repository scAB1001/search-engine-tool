...
@app.command("build", rich_help_panel="Database Operations")
def build(
...
@app.command("load", rich_help_panel="Database Operations")
def load() -> None:
...
@app.command("find", rich_help_panel="Search Operations")
def find(
...
@app.command("sitemap", rich_help_panel="General Utilities")
def sitemap(
    output_file: Annotated[str, typer.Option(
        "--output", "-o", help="The output XML file path.")] = "sitemap.xml"
) -> None:
    """
    [cyan]Generate[/cyan] an XML Sitemap by dynamically verifying live HTTP headers.
    """
    crawler = PoliteCrawler()
    ...
    with console.status(
        ...
        for url in sorted_urls:
            ...
            # 2. Polite HEAD request using the crawler
            headers = crawler.fetch_headers(url)
            if headers:
                ...
                if last_mod_header:
                    ...
                    lastmod = datetime.now(UTC).strftime("%Y-%m-%d")  # pragma: no cover
            else:
            ...
            # (No extra sleep as politeness is handled inside fetch_headers)
...
@app.command("show-sitemap", rich_help_panel="General Utilities")
def show_sitemap(
    sitemap_file: Annotated[
        Path | None,
        typer.Option("--file", "-f",
            help="""Path to the sitemap XML file. If no --file is given,
            the command searches for the most recent .xml file in the 'data/' directory.
